...
bool Vault_parser::retrieve_list(const Secure_string &payload, const Secure_string &list_name, Secure_string *list)
{
  return retrieve_tag_value(payload, list_name, '[', ']', list);
}

bool Vault_parser::retrieve_map(const Secure_string &payload, const Secure_string &map_name, Secure_string *map)
{
...
bool Vault_parser::get_vault_version(const Vault_credentials &vault_credentials, 
  ...
  if (is_null_tag(options) || is_empty_map(options)) {
    vault_version = 1; // version == null or empty map means we are using version 1
    return false;
  }
...
bool Vault_parser::parse_keys(const Secure_string &payload, Vault_keys_list *keys)
{
...
bool Vault_parser::parse_key_signature(const Secure_string &base64_key_signature, KeyParameters *key_parameters)
{
  ...
  for (int i= 0; i < 2; ++i)
  {
    size_t key_id_pos = key_signature.find_first_not_of(digits, next_pos_to_start_from);
    if (key_id_pos == Secure_string::npos || key_signature[key_id_pos] != '_')
      return true;
    ++key_id_pos;
    Secure_string key_id_length = key_signature.substr(next_pos_to_start_from, key_id_pos);
...
bool Vault_parser::parse_key_data(const Secure_string &payload, IKey *key)
{
  ...
  key->set_key_data(const_cast<uchar*>(reinterpret_cast<const uchar*>(decoded_key_data)),
