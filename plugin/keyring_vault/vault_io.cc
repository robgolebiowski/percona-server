...
bool Vault_io::adjust_vault_version(const Vault_credentials &vault_credentials) {
  int vault_version = 0;
  if (vault_credentials.get_credential("secret_mount_point_version") == "AUTO") {
    ...
    try 
    {
      vault_version = boost::lexical_cast<int>(vault_credentials.get_credential("secret_mount_point_version"));
