...
const Secure_string& Vault_credentials::get_credential(const Secure_string &key) const {
  Vault_credentials::Map::const_iterator it = vault_credentials.find(key);
...
Secure_string Vault_credentials::get_raw_secret_mount_point() const {
  ...
  return slash_separator != std::string::npos ? secret_mount_point_path.substr(0, slash_separator)
