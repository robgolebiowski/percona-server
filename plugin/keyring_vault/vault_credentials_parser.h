...
namespace keyring
{
  ...
  public:
      ...
      : logger(logger)
    {
      ...
      vault_credentials_in_progress.insert(std::make_pair("secret_mount_point_version", ""));

      value_options.insert(std::make_pair("vault_url", new Value_options(false)));
      value_options.insert(std::make_pair("secret_mount_point", new Value_options(false)));
      ...
      value_options.insert(std::make_pair("secret_mount_point_version", new Value_int_options(true,
                                                                               boost::make_optional<Secure_string>("1"),
                                                                               boost::make_optional(1),
                                                                               boost::make_optional(2))));

      //optional_value.insert("vault_ca");
      //optional_value.insert("secret_mount_point_version");
    }

    ~Vault_credentials_parser() {
      for(Value_options_map::iterator iter = value_options.begin(); iter != value_options.end();
          ++iter)
        delete iter->second;
    }
  ...
  private:
    ...
    class Value_options {
      public:
        Value_options(bool is_optional, boost::optional<Secure_string> default_value = boost::none)
        ...
        virtual bool process_value_options(const Secure_string &option_name, Secure_string &value, ILogger *logger);
    ...
    class Value_int_options : public Value_options {
      public:
        Value_int_options(bool is_optional, boost::optional<Secure_string> default_value,
                          boost::optional<int> min_value, boost::optional<int> max_value)
          : Value_options(is_optional, default_value), min_value(min_value), max_value(max_value)
          {}
    ...
    bool check_value_constrains(const Secure_string &option, Secure_string &value);

    Vault_credentials::Map vault_credentials_in_progress;

    typedef std::map<Secure_string, Value_options *> Value_options_map;
    //std::map<Secure_string, Value_options *> value_options;
