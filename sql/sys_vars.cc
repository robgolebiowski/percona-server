...
static const char *default_table_encryption_names[] = {
    "OFF", "ON", "KEYRING_ON", "ONLINE_TO_KEYRING", "ONLINE_FROM_KEYRING_TO_UNENCRYPTED"};

bool Sys_var_enum_default_table_encryption::global_update(THD *thd, set_var *var) { //TODO: Test if it deos not overflow

  if (thd->variables.default_table_encryption == var->save_result.ulonglong_value) 
    return 0;
...
static Sys_var_enum_default_table_encryption Sys_default_table_encryption(
    ...
    default_table_encryption_names, DEFAULT(DEFAULT_TABLE_ENC_OFF), NO_MUTEX_GUARD,
    IN_BINLOG, ON_CHECK(check_set_default_table_encryption_access));
