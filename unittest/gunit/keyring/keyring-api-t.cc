...
namespace keyring__api_unittest
{
  ...
  TEST_F(Keyring_api_test, GeneratePBFetchPBRotatePBFetchPB_UUID_version)
  {
    ...
    EXPECT_EQ(mysql_key_generate(percona_binlog_key_id_with_uuid.c_str(), "AES", NULL, 16), 0);

    char *key_type= NULL;
    size_t key_len= 0;
    void *key_ver1= NULL;
    EXPECT_EQ(mysql_key_fetch(percona_binlog_key_id_with_uuid.c_str(), &key_type, NULL, &key_ver1,
                              &key_len), 0);
    EXPECT_STREQ("AES", key_type);
    EXPECT_EQ(key_len, static_cast<size_t>(18));
    ASSERT_TRUE(memcmp(reinterpret_cast<char*>(key_ver1), "1:", 2) == 0);
    ...
    EXPECT_EQ(mysql_key_fetch(percona_binlog_key_id_with_uuid.c_str(), &key_type, NULL, &key_ver2,
                              &key_len), 0);
    EXPECT_STREQ("AES", key_type);
    EXPECT_EQ(key_len, static_cast<size_t>(18));
    ASSERT_TRUE(memcmp(reinterpret_cast<char*>(key_ver2), "2:", 2) == 0);
