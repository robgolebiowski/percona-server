...
namespace keyring__vault_credentials_parser_unittest
{
  ...
  TEST_F(Vault_credentials_parser_test, ParseFileWithoutVaultCA)
  {
    ...
    EXPECT_STREQ(vault_credentials.get_credential("vault_url").c_str(), "http://127.0.0.1:8200");
    EXPECT_STREQ(vault_credentials.get_credential("secret_mount_point").c_str(), "secret");
    EXPECT_STREQ(vault_credentials.get_credential("token").c_str(), "123-123-123");
    EXPECT_TRUE(vault_credentials.get_credential("vault_ca").empty());
  }
  ...
  TEST_F(Vault_credentials_parser_test, ParseFileWithCorrectCredentials)
  {
    ...
    EXPECT_STREQ(vault_credentials.get_credential("secret_mount_point").c_str(), "secret");
    EXPECT_STREQ(vault_credentials.get_credential("token").c_str(), "123-123-123");
    EXPECT_STREQ(vault_credentials.get_credential("vault_ca").c_str(), "/some/path");
    EXPECT_STREQ(vault_credentials.get_credential("secret_mount_point_version").c_str(), "1");
  }

  TEST_F(Vault_credentials_parser_test, ParseFileWithCorrectCredentialsWithSecretMountPointVersion2)
  {
    Vault_credentials_parser vault_credentials_parser(logger);
    std::string token;
    ...
    EXPECT_STREQ(vault_credentials.get_credential("token").c_str(), "123-123-123");
    EXPECT_STREQ(vault_credentials.get_credential("vault_ca").c_str(), "/some/path");
    EXPECT_STREQ(vault_credentials.get_credential("secret_mount_point_version").c_str(), "2");
  }

  TEST_F(Vault_credentials_parser_test, ParseFileWithCorrectCredentialsWithSecretMountPointVersionAUTO)
  {
    Vault_credentials_parser vault_credentials_parser(logger);
    std::string token;
    ...
    EXPECT_STREQ(vault_credentials.get_credential("token").c_str(), "123-123-123");
    EXPECT_STREQ(vault_credentials.get_credential("vault_ca").c_str(), "/some/path");
    EXPECT_STREQ(vault_credentials.get_credential("secret_mount_point_version").c_str(), "AUTO");
  }

  TEST_F(Vault_credentials_parser_test, ParseFileWithCorrectCredentialsWithSecretMountPointVersionAUTO2)
  {
    Vault_credentials_parser vault_credentials_parser(logger);
    std::string token;
  ...
  TEST_F(Vault_credentials_parser_test, ParseFileWithWithSecretMountVersionTooBig)
  {
    Vault_credentials_parser vault_credentials_parser(logger);
    std::string token;
    ...
    EXPECT_CALL(*(reinterpret_cast<Mock_logger*>(logger)),
      log(MY_ERROR_LEVEL, StrEq("Incorrect value for secret_mount_point_version. It is greater than max value = 2.")));
    std::string file_url = "./credentials";
    ...
    EXPECT_TRUE(vault_credentials.get_credential("secret_mount_point").empty());
    EXPECT_TRUE(vault_credentials.get_credential("vault_ca").empty());
    EXPECT_TRUE(vault_credentials.get_credential("secret_mount_point_version").empty());
  }

  TEST_F(Vault_credentials_parser_test, ParseFileWithWithSecretMountVersionTooBig2)
  {
    Vault_credentials_parser vault_credentials_parser(logger);
    std::string token;
    ...
    EXPECT_CALL(*(reinterpret_cast<Mock_logger*>(logger)),
      log(MY_ERROR_LEVEL, StrEq("Could not parse value for secret_mount_point_version from the configuration file.")));
    std::string file_url = "./credentials";
  ...
  TEST_F(Vault_credentials_parser_test, ParseFileWithWithSecretMountVersionTooSmall2)
  {
    Vault_credentials_parser vault_credentials_parser(logger);
    std::string token;
    ...
    EXPECT_CALL(*(reinterpret_cast<Mock_logger*>(logger)),
      log(MY_ERROR_LEVEL, StrEq("Incorrect value for secret_mount_point_version. It is smaller than min value = 1.")));
    std::string file_url = "./credentials";
  ...
  TEST_F(Vault_credentials_parser_test, ParseFileWithWithSecretMountVersionBeingString)
  {
    Vault_credentials_parser vault_credentials_parser(logger);
    std::string token;
  ...
  TEST_F(Vault_credentials_parser_test,ParseFileWithValuesWithSpacesInIt)
  {
    ...
    EXPECT_STREQ(vault_credentials.get_credential("vault_url").c_str(), "http: //127 .0.0.1: 8200");
    EXPECT_STREQ(vault_credentials.get_credential("secret_mount_point").c_str(), "s-e c-r -e t");
    EXPECT_STREQ(vault_credentials.get_credential("token").c_str(), "12000 3-10  23- 123");
    EXPECT_STREQ(vault_credentials.get_credential("vault_ca").c_str(), "/some/  path");
  }
}
