...
namespace keyring__vault_parser_unittest
{
  ...
  {
    Secure_string payload("\"secret/\": { \"accessor\": \"kv_3f89594e\","
                          ...
                          "\"seal_wrap\": false, \"type\": \"kv\", \"uuid\": \"2a825032-a2e7-8af8-e2cd-b93bd5deeb00\"},");
    ...
    Vault_parser vault_parser(logger);
    int vault_version = -1;
    EXPECT_FALSE(vault_parser.get_vault_version(vault_credentials, payload, vault_version));
    EXPECT_TRUE(vault_version == 2);
  }

  TEST_F(Vault_parser_test, GetVaultVersion1)
  {
    Secure_string payload("\"cicd/\": { \"accessor\": \"kv_3f89594e\","
                          "\"config\": { \"default_lease_ttl\": 0, \"force_no_cache\": false,"
                          "\"max_lease_ttl\": 0}, \"description\": \"key/value secret storage\","
