...
bool Encryption::none_explicitly_specified(ulong create_info_used_fields, const char *algorithm) noexcept {
...
bool Encryption::should_be_keyring_encrypted(ulong create_info_used_fields, const char *algorithm) {
  return !none_explicitly_specified(create_info_used_fields, algorithm) &&
         (is_keyring(algorithm) ||
         (!Encryption::is_master_key_encryption(algorithm) && is_online_encryption_on()));
}
...
void innobase_fix_default_table_encryption(ulong encryption_option) {
  if (!srv_read_only_mode) {
    fil_crypt_set_encrypt_tables(static_cast<enum_default_table_encryption>(encryption_option));
  }
}
...
static int innobase_init_files(dict_init_mode_t dict_init_mode,
  ...
  is_system_tablespace_encrypted = do_encrypt;
...
void ha_innobase::adjust_encryption_key_id(HA_CREATE_INFO *create_info,
  ...
  if (false == create_info->was_encryption_key_id_set) {
    if (Encryption::should_be_keyring_encrypted(create_info->used_fields,
  ...
  } else if (Encryption::is_master_key_encryption(create_info->encrypt_type.str) ||
...
static bool innobase_ddse_dict_init(
  ...
  bool is_system_tablespace_encrypted{false};

  if (innobase_init_files(dict_init_mode, tablespaces, is_system_tablespace_encrypted)) {
    DBUG_RETURN(true);
  }
  ...
  */
  ...
  if (is_system_tablespace_encrypted) {
