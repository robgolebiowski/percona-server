	...
	struct st_mysql_value*		value)	/*!< in: incoming string */
{
	...
	if(!Encryption::tablespace_key_exists_or_create_new_one_if_does_not_exist(static_cast<uint>(intbuf), server_uuid)) {
		push_warning_printf(thd, Sql_condition::SL_WARNING,
				    HA_ERR_UNSUPPORTED,
	...
	void	*p)	/*!< in: InnoDB handlerton */
{
	...
	if ((srv_encrypt_tables == SRV_ENCRYPT_TABLES_ONLINE_TO_KEYRING || srv_encrypt_tables == SRV_ENCRYPT_TABLES_ONLINE_TO_KEYRING_FORCE)
	    && !Encryption::tablespace_key_exists_or_create_new_one_if_does_not_exist(FIL_DEFAULT_ENCRYPTION_KEY, server_uuid)) {
...
create_table_info_t::enable_keyring_encryption(dict_table_t *table, fil_encryption_t &keyring_encryption_option) {
	...
	if ((check_keyring_key || m_create_info->was_encryption_key_id_set) &&
      	    !Encryption::tablespace_key_exists_or_create_new_one_if_does_not_exist(m_create_info->encryption_key_id, server_uuid)) {
