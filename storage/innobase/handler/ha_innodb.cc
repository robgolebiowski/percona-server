...
static int innobase_alter_encrypt_tablespace(handlerton *hton, THD *thd,
  ...
  if (!is_old_encrypted && !is_new_encrypted && alter_info->explicit_encryption) {
    ...
    // TODO: PS-5323 should disallow reaching this code if encryption threads are enabled.
    // Thus it should be safe to remove the crypt_data here.
    if (space->crypt_data) {
    ...
    if (space->crypt_data) {
      my_error(ER_EXPLICIT_DECRYPTION_OF_ONLINE_ENCRYPTED_TABLESPACE, MYF(0), space->name);
      DBUG_RETURN(convert_error_code_to_mysql(DB_ERROR, 0, NULL));
    }
