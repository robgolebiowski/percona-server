...
static void fil_crypt_return_iops(rotate_thread_t *state, bool set_fil_crypt_threads_event = true) {
  if (state->allocated_iops > 0) {
    ...
    if (set_fil_crypt_threads_event)
      os_event_set(fil_crypt_threads_event);
    mutex_exit(&fil_crypt_threads_mutex);
  }
...
static bool fil_crypt_find_space_to_rotate(key_state_t *key_state,
  ...
  /* If we didn't find any space return iops. Do not set fil_crypt_threads_event.
     We do not want to notify other threads that there is some work to do - since
